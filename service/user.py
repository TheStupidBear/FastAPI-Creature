...
def login_for_access_token(username:str, password:str) -> Token:
    if data.login_user(username, password): #если есть пользователь в БД
        access_token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)  # время действия токена
